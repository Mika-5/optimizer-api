...
require './wrappers/cplex_vrp_pb'
require './wrappers/cplex_result_pb'
require 'tempfile'
require 'json'
...
module Wrappers
  class Cplex < Wrapper
    def initialize(cache, hash = {})
      super(cache, hash)
      @exec_cplex = hash[:exec_cplex] || 'LD_LIBRARY_PATH=../optimizer-cplex/dependencies/install/lib/:../CPLEX/cpoptimizer/lib/x86-64/static_pic/ ../optimizer-cplex/tsp_cplex'
    ...
    def solve(vrp, job, thread_proc = nil, &block)
      ...
      services = vrp.services.collect{ |service|
        CplexVrp::Service.new(
          time_windows: service.activity.timewindows.collect{ |tw| CplexVrp::TimeWindow.new(
            start: tw.start || -2**56,
            end: tw.end || 2**56,
          )},
      ...
      vehicles = vrp.vehicles.sort!{ |a, b|
        ...
        CplexVrp::Vehicle.new(
          ...
          time_window: CplexVrp::TimeWindow.new(
      ...
      ret = run_cplex(problem, vrp, services, points, matrix_indices, thread_proc, &block)
      case ret
      when String
        return ret
      when Array
    ...
    def parse_output(vrp, services, points, matrix_indices, cost, output)
      ...
      content = CplexResult::Result.decode(output.read)
      output.rewind
      ...
      collected_rests_indices = []
      {
        ...
        routes: content['routes'].each_with_index.collect{ |route, index|
          ...
          activities: route['activities'].collect{ |activity|
            ...
            if activity['type'] == 'start' && activity['index'] == 0
            ...
            elsif activity['type'] == 'end' && activity['index'] == 0
              {
                point_id: vrp.points[current_index].id,
              ...
              current_activity = {
                service_id: vrp.services[current_index].id,
    ...
    def run_cplex(problem, vrp, services, points, matrix_indices, thread_proc = nil, &block)
      logger = Logger.new(STDERR)
      if vrp.vehicles.size == 0 || (vrp.services.nil? || vrp.services.size == 0) && (vrp.shipments.nil? || vrp.shipments.size == 0)
        return [0, 0, @previous_result = parse_output(vrp, services, points, matrix_indices, 0, nil)]
      end
      encoded_vrp = CplexVrp::Problem.encode(problem)
      input = Tempfile.new('optimize-cplex-input', tmpdir=@tmp_dir)
      input.write(encoded_vrp)
      input.close

      output = Tempfile.new('optimize-cplex-output', tmpdir=@tmp_dir)
