...
# FIXME or-tools can handle no end-point itself
    ...
    def build_detail(job, activity, point, day_index, job_load, vehicle, delivery=nil)
      {
        ...
        additional_value: activity && activity.additional_value && vehicle.cost_value_multiplier && (activity.additional_value * vehicle.cost_value_multiplier),
      ...
      collected_rests_indices = []
      {
        ...
        unassigned: (vrp.services.collect(&:id) - collected_indices.collect{ |index| index < vrp.services.size && vrp.services[index].id }).collect{ |service_id|
          service = vrp.services.find{ |service| service.id == service_id }
          {
            ...
            detail: service.activity ? build_detail(service, service.activity, service.activity.point, nil, nil, vrp.vehicles[0]) : {activities: service.activities}
          }
          ...
          [{
            ...
            type: 'pickup',
            point_id: shipment.pickup.point_id,
            detail: build_detail(shipment, shipment.pickup, shipment.pickup.point, nil, nil, vrp.vehicles[0])
          }, {
            shipment_id: "#{shipment_id}",
            type: 'delivery',
            point_id: shipment.delivery.point_id,
            detail: build_detail(shipment, shipment.delivery, shipment.delivery.point, nil, nil, vrp.vehicles[0])
          }]
    ...
    def run_ortools(problem, vrp, services, points, matrix_indices, thread_proc = nil, &block)
      ...
      cmd = [
        ...
        vrp.resolution_only_first_solution ? "-only_first_solution #{vrp.resolution_only_first_solution}" : nil,
      ...
      if @thread.value == 0
        ...
        raise RuntimeError.new(result) unless vrp.resolution_only_first_solution
      end
    ensure
      input && input.unlink
