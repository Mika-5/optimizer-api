...
module Interpreters
  class Dichotomious
    ...
    def self.split(service_vrp, job = nil)
      ...
      services_by_cluster = kmeans(vrp, :duration)
      services_by_cluster.sort_by!{ |ss| Helper.services_duration(ss) } if !services_by_cluster.second.compact.empty?
      split_service_vrps = []
      if !services_by_cluster.second.compact.empty?
